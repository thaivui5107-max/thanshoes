  ...
  const getComboMeta = (combo: any) => {
    ...
    if (combo.type === 'standard') {
      ...
      if (cfg?.rewardType === 'discount_percent') {
        rewardLabel = `Giảm ${cfg.rewardValue}%`;
      } else if (cfg?.rewardType === 'discount_amount') {
        rewardLabel = `Giảm ${formatPrice(cfg.rewardValue || 0)}`;
        ...
        rewardLabel = `Tặng kèm ${cfg.giftQty || 1} ${giftProduct?.name || 'Sản phẩm khác'}`;
      }
    } else if (combo.type === 'mix') {
    ...
    return {
      ...
      priceLabel: combo.price ? formatPrice(combo.price) : 'Liên hệ',
      rewardLabel,
    };
  };

  const renderComboCard = (combo: any, index: number) => {
    const meta = getComboMeta(combo);
    const mainText = meta.name || meta.conditionLabel || meta.rewardLabel || 'Combo ưu đãi';

    return (
      <div
        className={`group relative flex min-h-12 items-center gap-2 overflow-hidden rounded-xl border px-3 py-2 transition-all hover:shadow-md ${animateClass}`}
        ...
        <div className="min-w-0 flex-1 overflow-hidden" ref={emblaRef}>
          <div className="flex max-h-16 flex-col">
            {combos.map((combo, index) => (
              <div key={combo.id ?? index} className="min-w-0 flex-[0_0_100%]">
