...
- Nhu cầu thật chỉ là: bật/tắt tính năng và upload **1 ảnh khung PNG/WebP nền trong suốt** để đè lên ảnh sản phẩm.
...
- Khi ảnh sản phẩm và ảnh khung cùng Aspect Ratio (Tỷ lệ khung hình - AR), ví dụ đều `1:1`, chỉ cần overlay `absolute inset-0 object-contain` là đủ.
...
Giải pháp tốt hơn là chuyển phần "thiết kế khung" ra ngoài hệ thống, còn hệ thống chỉ làm đúng việc nó cần làm: lưu 1 URL ảnh khung, kiểm tra toggle, rồi render ảnh khung nằm trên ảnh sản phẩm. Đây là hướng KISS/YAGNI, ít code hơn, dễ vận hành hơn, ít điểm lỗi hơn.

Điểm quan trọng: **không cần hệ thống tự sửa AR**. Admin chỉ cần dùng ảnh khung có cùng AR với ảnh sản phẩm đang dùng. Nếu site đang dùng ảnh sản phẩm vuông `1:1`, ảnh khung cũng nên là `1:1` như `1000x1000`. Nếu sau này cần hỗ trợ nhiều AR, lúc đó mới mở rộng thêm nhiều ảnh khung theo AR; hiện tại không làm.

## 3. Concrete Examples & Analogies

- **Ví dụ bám sát dự án:** Shop muốn chạy campaign Tết. Designer tạo file `frame-tet-1000x1000.png`, giữa trong suốt, bốn góc có hoa văn. Admin vào `/admin/settings/advanced` → tab **Khung viền sản phẩm** → upload ảnh này. Sau đó vào `/system/modules/products` bật toggle **Bật khung viền sản phẩm**. Tất cả card sản phẩm dùng khung vuông sẽ hiển thị frame Tết.
...
- Hệ thống hiện tại phức tạp vì đang model hóa "nhiều khung + nhiều kiểu tạo khung", trong khi nhu cầu hiện tại là "1 ảnh overlay global".
- `settings` table phù hợp hơn `productImageFrames` cho flow mới vì đây là cấu hình global, giống `product_image_placeholder`.
...
- Triển khai theo hướng **Simple Overlay**:
  ...
  - Advanced settings quản lý 1 ảnh overlay.
  - Renderer dùng ảnh overlay từ `settings`.
...
- `ProductFrameManager` xử lý nhiều nhánh UX và nhiều loại source (`uploaded_overlay`, `line_generator`, `logo_generator`) trong khi yêu cầu thực tế chỉ cần upload 1 ảnh.
- `productImageFrames` là table riêng cho nhiều record, nhưng bài toán mới chỉ cần 1 setting global.
...
- **Đánh giá:** Chưa đủ evidence từ nhu cầu hiện tại. Nếu làm tiếp manager nhiều frame sẽ tiếp tục tăng độ phức tạp. Khi thật sự cần nhiều seasonal frame, có thể mở rộng từ simple overlay bằng thêm `product_frame_overlay_*` hoặc preset sau.
...
4. **Dữ liệu thiếu:** Chưa kiểm tra dữ liệu production đang có bao nhiêu `productImageFrames`; trước khi xóa cần migrate tối thiểu active uploaded overlay cũ sang `product_frame_overlay_url` nếu có.
...
- Đơn giản hóa Product Frames thành **1 global overlay image**.
- Giữ UI dễ hiểu:
  - `/system/modules/products`: chỉ bật/tắt.
  - `/admin/settings/advanced`: upload/xóa/preview ảnh khung.
- Làm sạch hệ thống:
...
Lưu trong `settings` table bằng flow hiện có:

- `product_frame_overlay_url`
  - group: `advanced`
  - value: URL ảnh overlay
- `product_frame_overlay_url__storageId`
  - group: `advanced`
  - value: Convex storage id
  - dùng cùng pattern `SETTING_STORAGE_ID_SUFFIX = "__storageId"` trong `convex/settings.ts`
...
Các key/table legacy trên cần được gỡ khỏi config/code/schema. Nếu production đang có `activeProductFrameId` trỏ tới frame loại `uploaded_overlay`, migrate URL đó sang `product_frame_overlay_url` trước khi bỏ table. Các frame loại `line_generator`/`logo_generator` không migrate vì không còn thuộc scope mới.
...
- Trong tab này có:
  - `SettingsImageUploader` label `Ảnh khung viền sản phẩm`
  - mô tả ngắn: `Upload ảnh PNG/WebP nền trong suốt. Nên dùng cùng tỷ lệ với ảnh sản phẩm, ví dụ 1:1.`
  - preview khung đè lên ảnh placeholder/sample vuông
  - nút `Xóa khung`
- Nếu `enableProductFrames` đang tắt, vẫn cho upload ảnh nhưng hiển thị callout nhẹ: `Tính năng đang tắt trong Module Products.`
...
2. Query `settings.product_frame_overlay_url`.
3. Nếu `enabled === true` và URL có giá trị:
   - render overlay image.
4. Nếu thiếu 1 trong 2:
   - không render frame.
...
1. Thêm setting mới `product_frame_overlay_url`.
2. Nếu cần giữ cấu hình đang dùng, đọc `activeProductFrameId`:
   - nếu frame active là `uploaded_overlay` và có `overlayImageUrl`, copy sang `settings.product_frame_overlay_url`;
   - copy `overlayStorageId` sang `product_frame_overlay_url__storageId` nếu có;
   - bỏ qua `line_generator`/`logo_generator` vì flow mới chỉ nhận ảnh overlay.
3. Renderer chuyển sang đọc setting mới.
...
- **Sửa:** `app/admin/settings/_components/SettingsPageShell.tsx`  
  ...
  Thay đổi: thêm `AdvancedTab = 'product-placeholder' | 'product-frame' | 'header'`, thêm uploader/preview cho `product_frame_overlay_url`, đảm bảo save kèm storage id.
...
- **Xóa:** `app/admin/settings/_components/ProductFrameManager.tsx`  
  ...
  Thay đổi: xóa khỏi codebase, vì flow mới chỉ upload 1 ảnh trong Advanced settings.
...
- **Sửa:** `components/shared/ProductImageFrameBox.tsx`  
  ...
  Thay đổi: query `enableProductFrames` và `settings.getValue/getMultiple` cho `product_frame_overlay_url`; trả về overlay URL hoặc object đơn giản, render `<img>` overlay duy nhất.
...
- **Review giữ tương thích:** các call site đang import `ProductImageFrameOverlay`/`useProductFrameConfig`:
  ...
  Thay đổi: ưu tiên giữ API component/hook gần giống để giảm số file phải sửa; nếu đổi shape thì cập nhật đồng bộ.
...
- **Review:** `convex/media.ts`  
  ...
  Thay đổi: xóa scan usage của `productImageFrames`; đảm bảo `settings.product_frame_overlay_url__storageId` vẫn được File Lifecycle Service theo pattern settings hiện có.
...
4. Thêm save/load `product_frame_overlay_url` theo pattern `product_image_placeholder`.
...
8. Refactor `ProductImageFrameBox` để dùng setting overlay URL thay vì `activeProductFrameId + productImageFrames`.
...
- Vào `/admin/settings/advanced`:
  ...
  - upload được ảnh PNG/WebP khung.
  - save xong refresh vẫn còn ảnh.
- Khi toggle off:
  - storefront/admin preview không render overlay.
- Khi toggle on + có ảnh overlay:
  - product card/list/detail render khung.
- Với ảnh sản phẩm `1:1` và frame `1:1`:
  - overlay khít, không crop, không che interaction vì `pointer-events-none`.

# VIII. Todo

- [ ] Sửa `lib/modules/configs/products.config.ts` chỉ còn toggle frame.
- [ ] Thêm tab `Khung viền sản phẩm` trong `SettingsPageShell`.
- [ ] Lưu `product_frame_overlay_url` và storage id trong `settings`.
- [ ] Redirect route cũ `/admin/settings/product-frames`.
- [ ] Bỏ menu sidebar khung sản phẩm.
- [ ] Refactor `ProductImageFrameBox` sang simple overlay.
- [ ] Review call sites giữ tương thích.
- [ ] Migrate active uploaded overlay cũ sang `product_frame_overlay_url` nếu dữ liệu cũ đang có.
...
- `/admin/settings/advanced` có tab **Khung viền sản phẩm** để upload/xóa/preview 1 ảnh overlay.
- Overlay image lưu bằng `settings.product_frame_overlay_url`.
- Storefront/admin preview chỉ render frame khi:
  - `enableProductFrames === true`
  - `product_frame_overlay_url` có URL hợp lệ
...
- Nếu không migrate active uploaded overlay cũ, cấu hình khung đang dùng có thể mất.
...
- Thêm hướng dẫn microcopy: ảnh khung nên cùng AR với ảnh sản phẩm, ví dụ `1:1`.
...
- Setting mới `product_frame_overlay_url` vẫn an toàn, có thể giữ lại ngay cả khi rollback UI.

# XI. Out of Scope (Ngoài phạm vi)

- Không làm crop editor trong browser.
- Không tự remove background ảnh upload.
- Không hỗ trợ nhiều khung theo mùa trong đợt triển khai này.
- Không tối ưu AR tự động hoặc sinh nhiều frame theo từng AR.
